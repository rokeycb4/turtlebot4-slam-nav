...
class CarPlateOCRNode(Node):
    def __init__(self):
        ...
        # 추가된 퍼블리셔
        self.bbox_img_pub = self.create_publisher(Image, '/carplate/debug/bbox_image', 10)
        self.cropped_img_pub = self.create_publisher(Image, '/carplate/debug/cropped_image', 10)
        ...
        self.get_logger().info(f"바운딩 박스 이미지 퍼블리셔: {self.bbox_img_pub.topic_name}") # 로그 추가
        self.get_logger().info(f"크롭된 이미지 퍼블리셔: {self.cropped_img_pub.topic_name}")   # 로그 추가
    ...
    def process_image_callback(self):
        ...
        img_bbox_only = self.latest_image.copy() # 바운딩 박스만 그릴 이미지 복사
        ...
        for r in results:
            for box in r.boxes:
                ...
                # 모든 검출된 객체에 바운딩 박스 그리기 (img_bbox_only용)
                cv2.rectangle(img_bbox_only, (x1, y1), (x2, y2), (0, 255, 0), 2)
                ...
                if class_name == "disabled":
                    ...
                    # CROP된 이미지 발행
                    try:
                        cropped_msg = self.bridge.cv2_to_imgmsg(roi, encoding='bgr8')
                        cropped_msg.header.stamp = self.get_clock().now().to_msg()
                        self.cropped_img_pub.publish(cropped_msg)
        ...
        # 바운딩 박스만 있는 이미지 발행
        try:
            bbox_msg = self.bridge.cv2_to_imgmsg(img_bbox_only, encoding='bgr8')
            bbox_msg.header.stamp = self.get_clock().now().to_msg()
            self.bbox_img_pub.publish(bbox_msg)
        except Exception as e:
            self.get_logger().error(f"Failed to publish bbox image: {e}")
