...
class DetectObjectsWithTF(Node):
    ...
    def process_image(self):
        ...
        for box, conf, class_id in zip(results.boxes.xyxy.cpu().numpy(),
            ...
            point_cam.header.frame_id = 'camera_link'  # 또는 '/robot3/oakd/stereo'
            ...
            try:
                ...
                self.get_logger().info(
                    f"[{self.class_names[int(class_id)]}] {conf:.2f} at camera({x:.2f}, {y:.2f}, {z:.2f}) → base({point_base.point.x:.2f}, {point_base.point.y:.2f}, {point_base.point.z:.2f})"
                )
            except Exception as e:
